...
typedef std::function<AssetPtr(const std::string& name, std::istream& stream,
  size_t size, RefResolver& resolver)> DeserializeBinaryProc;

typedef std::function<AssetPtr(const std::string& name, const std::string& path,
  RefResolver& resolver)> DeserializeFriendlyProc;
