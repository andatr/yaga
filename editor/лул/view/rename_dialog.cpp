...
{
public:
  RenameDialogImpl(GuiContext context, const std::string& value, Handle handle);
...
// -----------------------------------------------------------------------------------------------------------------------------
RenameDialogImpl::RenameDialogImpl(GuiContext context, const std::string& value, Handle handle) :
  Widget(context),
  handle_(handle),
  buffer_(512),
  size_{}
...
void RenameDialogImpl::render()
{
  ...
  ImGui::OpenPopup(LABEL("Rename"));
  ImGui::SetNextWindowPos((context_.screen() - size_) / 2.0f);
  if (!ImGui::BeginPopup(LABEL("Rename"))) {
    enqueueRendering();
    return;
  }
  ImGui::Text("Rename");
...
{
  auto impl = std::make_shared<RenameDialogImpl>(context, value, handle);
