...
// -------------------------------------------------------------------------------------------------------------------------
AutoDestructor<VkDeviceMemory> Allocator::allocateImage(VkImage image, VkMemoryPropertyFlags properties) const
{
  VkMemoryRequirements requirements;
  vkGetImageMemoryRequirements(vkDevice, image, &requirements);
  return allocate(requirements, properties);
}
