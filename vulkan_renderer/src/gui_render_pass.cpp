...
#include "vulkan_renderer/gui_render_pass.h"
#include "vulkan_renderer/gui_vertex.h"
...
// -----------------------------------------------------------------------------------------------------------------------------
GuiRenderPass::GuiRenderPass(Swapchain* swapchain, VmaAllocator allocator) :
...
void GuiRenderPass::render(uint32_t imageIndex)
{
  ...
  const auto indexCount = 0;// static_cast<uint32_t>(indices_.size());
...
void GuiRenderPass::createBuffers()
{
  ...
  const auto vertexSize = 0; // static_cast<VkDeviceSize>(sizeof(Vertex) * vertices_.size());
  const auto indexSize  = 0; // static_cast<VkDeviceSize>(sizeof(uint16_t) * indices_.size());
