...
Image::Image(Device* device, VmaAllocator allocator, const VkImageCreateInfo& info, VkImageViewCreateInfo viewInfo,
  ...
  vkDevice_(**device), allocator_(allocator), image_(VK_NULL_HANDLE), allocation_{}, memory_{},
  imageView_(VK_NULL_HANDLE), sampler_(sampler), info_(info)
{
  VULKAN_GUARD(vmaCreateImage(allocator_, &info, &allocInfo, &image_, &allocation_, nullptr), "Could not create Vulkan Image");
